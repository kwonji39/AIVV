...
class SystemEngineerAgent(LLMAgent):
    ...
    def _build_training_evidence_summary(
        ...
        train_error_q99 = float(train_probe.get('error_distribution', {}).get('q99', 0.0) or 0.0)
        train_sigma_q95 = float(train_probe.get('uncertainty_distribution', {}).get('q95', 0.0) or 0.0)
        ...
        ratios = {
            'error_to_train_q99': (current_error / train_error_q99) if train_error_q99 > 0 else None,
            'uncertainty_to_train_q95': (current_uncertainty / train_sigma_q95) if train_sigma_q95 > 0 else None,
            'step_to_train_q99': (current_step / step_q99) if step_q99 > 0 else None,
            'range_to_train_q99': (current_range / range_q99) if range_q99 > 0 else None,
        }
        ...
        for metric, value in ratios.items():
            if value is None:
                continue
            item = {'metric': metric, 'ratio': float(value)}
            if value <= 1.0:
                pass_support.append(item)
            else:
                fail_support.append(item)

        ranked_signals = sorted(
            pass_support + fail_support,
            key=lambda item: item['ratio'],
            reverse=True,
        )

        return {
            'train_error_q99': train_error_q99,
            'train_sigma_q95': train_sigma_q95,
            ...
            'ratios': ratios,
            'evidence_balance': {
                'supports_pass': pass_support,
                'supports_fail': fail_support,
                'support_counts': {
                    'pass_like': len(pass_support),
                    'fail_like': len(fail_support),
                },
